...
esp_err_t bme_init(i2c_master_dev_handle_t dev_handle) {
    ...
    esp_err_t err = i2c_master_transmit_receive(dev_handle, &reg_id, 1, &chip_id, 1, 500);         
    ...
    if ((err == ESP_OK) && (chip_id == 0x60)) {
        ...
        i2c_master_transmit(dev_handle, config_hum, 2, 50); 
        i2c_master_transmit(dev_handle, config_meas, 2, 50);
        return ESP_OK;
		}
    return ESP_FAIL;
	}
...
esp_err_t bme280_read_raw(int32_t *adc_T, int32_t *adc_H) {
    
    esp_err_t ret = i2c_master_transmit_receive(dev_handle, &start_reg, 1, buf, 5, 500);
    if (ret != ESP_OK) {
        return ret; 
    }
	...
	esp_err_t ret = i2c_master_transmit_receive(bme_handle, &start_reg, 1, buf, 5, 500);
    if (ret != ESP_OK) {
        return ret; 
    }
...
double bme_read_temp(int32_t adc_T) {
...
double bme_read_hum(int32_t adc_H) {
